...
class Agent():
    ...
    def move(self): 
        ...
        if self.am_i_happy_b == True:                           #if the agent is happy, dont move and return.
            if self.kind=="red":
                return 0 
            else:
                return 1
        
        else:                                                   #if the agent is unhappy, she will try to move.
            vacancies = self.world.find_vacant(return_all=True) #check all vacancies
            for vacancy in vacancies:                           
                if self.am_i_happy(loc=vacancy):                #check if she will be happy in this new location
                    self.world.grid[self.location]=None         #remove herself from the current location
                    self.location = vacancy                     #set new location on agent and world grid levels
                    self.world.grid[vacancy]=self
                    if self.kind=="red":                        
                        return 4 
                    else:
                        return 5 
            if self.kind=="red":                                #if no current vacancy is making the agent happy, do not move, return.
                return 2 
            else:
                return 3
    ...
    def am_i_happy(self, loc=False, neighbor_check=False):
        ...
        if loc==False:                                          #actual location or pretended location
            location = self.location
        else:
            location = loc
        
        #4 steps: get neighbors' locations; get their colors; check for similarity; verify overall happiness.
        neighbors_locations = self.world.locate_neighbors(location)
        neighbors_colors = [self.world.grid[location].kind if self.world.grid[location] != None else "empty" for location in neighbors_locations]
        neighbors_boolean = [colors==self.kind for colors in neighbors_colors ]
        self.am_i_happy_b = True if sum(neighbors_boolean)>=self.same_pref*len(neighbors_colors) else False
        
        if neighbor_check==True:                                #return list of neighbors similarity or overall happiness
            return neighbors_boolean
        else: 
            return self.am_i_happy_b
    
    def start_happy_r_b(self):
    #for reporting purposes, allow count of happy before any moves, of red and blue seperately
        if self.am_i_happy_b and self.kind == 'red':
            return 'a'
        elif self.am_i_happy_b and self.kind == 'blue':
            return 'b'
        else:
            pass
